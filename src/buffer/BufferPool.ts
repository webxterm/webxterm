 ...
 * BufferPool = [ Buffer = [block, block, block], Buffer, Buffer, Buffer, Buffer, ...]
 */
import {
    DataBlockAttribute
...
export class BufferPool {
    ...
    // 缓冲区
    private readonly _change_buffer: Buffer;
    // 保留区
    private readonly _saved_buffer: Buffer;
    // 显示区
    private readonly _display_buffer: Buffer;
    ...
    constructor(rows: number, columns: number, scrollBack: boolean, type: string = "") {
        ...
        this.x = 1;
        this._high_water = 1;
        // this.blankBlocks = Array.from({length:columns}, () => BufferPool.newBlankBlock());

        this._change_buffer = new Buffer(rows, type + "_change_buffer");
        this._saved_buffer = new Buffer(0, type + "_saved_buffer");
        this._display_buffer = new Buffer(0, type + "_display_buffer");
        // this._undo_buffer = new Buffer(0, "undo_buffer");
    }
    ...
    set x(value: number) {
        ...
        // console.info("y: " + this._y + ", x:" + this._x)
    }
    ...
    get size(): number {
        return this._change_buffer.lineChars.length;
    }
    ...
    // 获取当前光标的所在行编号
    get cursorLineId(): number{
        return this._change_buffer.lineIds[this._y - 1];
    }
    ...
    resize(newRows: number, newCols: number) {
        ...
        if (currentRows != newRows) {
            ...
            // 将需要的行移动到缓冲区
            let start = this._saved_buffer.lineChars.length - newRows;
            this._saved_buffer.moveLineTo(this._change_buffer, start < 0 ? 0 : start, newRows);

            // 当前缓冲区的行数
            const cur_buf_row_count = this._change_buffer.lineChars.length;
            if (currentRows < newRows) {
                // 窗口的高度放大。 24 -> 25
    ...
    removeLine(start: number, count: number, scroll_back: boolean = false): void {
        if (scroll_back && this.scrollBack) {
            ...
            if (this._saved_buffer.lineChars.length > this.maxScrollBack) {
                // 直接将第一行删除。
    ...
    eraseLine(y: number, blockAttr: DataBlockAttribute): void {
        if (!this._change_buffer.lineChars[y - 1]) {
            console.info("eraseLine:this._lines[y - 1]" + this._change_buffer.lineChars[y - 1]);
            return;
        }
        for (let xIndex = 0, len = this._change_buffer.lineChars[y - 1].length; xIndex < len; xIndex++) {
    ...
    erase(yIndex: number, xIndex: number, blockAttr: DataBlockAttribute) {
        const block = this._change_buffer.lineChars[yIndex][xIndex];
        if(block == undefined) {
            return;
        }
